...
function createDirs(appname) {
	fs.mkdirp(path.join(appname ,'actions/core/'), function (err) {
	  if (err) {
	    console.error(err);
	  } else {
	  	copyTemplate('action', appname);
		fs.mkdirp(path.join(appname ,'components/core/'), function (err) {
		  if (err) {
		    console.error(err);
		  } else {
	  		copyTemplate('component', appname);
		  	fs.mkdirp(path.join(appname ,'constants/core/'), function (err) {
			  if (err) {
			    console.error(err);
			  } else {
		  		copyTemplate('constant', appname);
			  	fs.mkdirp(path.join(appname ,'stores/core/'), function (err) {
				  if (err) {
				    console.error(err);
				  } else {
