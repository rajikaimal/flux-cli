...
function copyPackageJson(appname) {
	fs.copy('templates/package.json', '../' + appname + '/package.json', { replace: true }, function (err) {
	  if (err) {
	    // i.e. file already exists or can't write to directory 
	    throw err;
	  }
...
function copyTemplate(type, appname) {
	if(type === 'action') {
		...
		fs.copy('templates/components/component.js', '../' + appname + '/components/core/component.js', { replace: true }, function (err) {
		  if (err) {
		    // i.e. file already exists or can't write to directory 
		    throw err;
		  }
